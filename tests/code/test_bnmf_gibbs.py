...
def test_init():
    ...
    R3 = numpy.ones((3,2))
    with pytest.raises(AssertionError) as error:
        bnmf_gibbs(R3,M,K,ARD,hyperparams)
    ...
    with pytest.raises(AssertionError) as error:
        bnmf_gibbs(R4,M,K,False,hyperparams)
    ...
    with pytest.raises(AssertionError) as error:
        bnmf_gibbs(R4,M1,K,ARD,hyperparams)
    assert str(error.value) == "Fully unobserved row in R, row 1."
    with pytest.raises(AssertionError) as error:
        bnmf_gibbs(R4,M2,K,ARD,hyperparams)
    ...
    M = numpy.ones((I,J))
    hyperparams = { 'alphatau':alphatau, 'betatau':betatau, 'alpha0':alpha0, 'beta0':beta0 }
    BNMF = bnmf_gibbs(R5,M,K,ARD,hyperparams)
    ...
    M = numpy.ones((I,J))
    hyperparams = { 'alphatau':alphatau, 'betatau':betatau, 'lambdaU':lambdaU, 'lambdaV':lambdaV }
    BNMF = bnmf_gibbs(R5,M,K,False,hyperparams)
...
def test_initialise():
    ...
    init_UV = 'random'
    BNMF = bnmf_gibbs(R,M,K,True,priors)
    ...
    init_UV = 'exp'
    BNMF = bnmf_gibbs(R,M,K,False,priors)
    BNMF.initialise(init_UV)
...
def test_alpha_s():
    BNMF = bnmf_gibbs(R,M,K,False,hyperparams)
...
def test_alphak_s():
    BNMF = bnmf_gibbs(R,M,K,True,hyperparams)
    BNMF.initialise(init)
    alphak_s = alpha0 + I + J
    for k in range(K):

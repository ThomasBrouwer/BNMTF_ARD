...
plot_file = folder_plots+"mse_nmtf_ard_noise.png"
...
icm_ard_performances = eval(open(folder_results+'nmtf_icm_ard.txt','r').read())
np_performances = eval(open(folder_results+'nmtf_np.txt','r').read())


''' Assemble the average performances and method names. '''
methods = ['VB-NMTF', 'VB-NMTF (ARD)', 'G-NMTF', 'G-NMTF (ARD)', 'ICM-NMTF', 'ICM-NMTF (ARD)', 'NP-NMTF']
performances = [
    vb_performances,
    vb_ard_performances,
    gibbs_performances,
...
for metric in metrics:
    ...
    fig.add_subplot(111)
    fig.subplots_adjust(left=0.11, right=0.99, bottom=0.17, top=0.95)
    ...
    plt.ylim(0,MSE_max)
    plt.yticks(range(0,MSE_max,100))
    plt.xlim(0,11.3)
